...
const getCompanyId = (context = {}) =>
  context.companyId || context.user?.companyId || null;
...
const listProjects = async (query = {}, context = {}) => {
  ...
  if (!companyId) {
    throw createError(
      "COMPANY_REQUIRED",
      "Company is required to list projects.",
      400,
      "companyId"
    );
  }
...
const createProject = async (payload, context = {}) => {
  ...
  if (!companyId) {
    throw createError(
      "COMPANY_REQUIRED",
      "Company is required to create a project.",
      400,
      "companyId"
    );
  }
  ...
  const existing = workspaceId
    ? await projectRepository.findProjectByKey(
        companyId,
        workspaceId,
        validated.key
      )
...
const updateProject = async (projectId, payload, context = {}) => {
  ...
  if (validated.key && validated.key !== project.key) {
    const existing = await projectRepository.findProjectByKey(
      companyId,
      workspaceId,
      validated.key
    );
...
const deleteProject = async (projectId, context = {}) => {
  ...
  const deleted = await projectRepository.deleteProject(
    projectId,
    context.user?.id,
    getCompanyId(context),
    getWorkspaceId(context)
...
const restoreProject = async (projectId, context = {}) => {
  ...
  const existing = await projectRepository.findProjectById(
    projectId,
    getWorkspaceId(context),
    true,
    getCompanyId(context)
  ...
  const restored = await projectRepository.restoreProject(
    projectId,
    getCompanyId(context),
    getWorkspaceId(context)
  );
...
const listProjectMembers = async (projectId, query = {}, context = {}) => {
  ...
  const result = await projectRepository.listProjectMembers({
    projectId,
    page,
    pageSize,
    role,
