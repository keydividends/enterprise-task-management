...
export const UserListPage = () => {
  ...
  const canCreate =
    isAdminOrManager ||
    currentUser?.permissions?.includes('USER_CREATE');
  ...
  const canDelete =
    isAdminOrManager ||
    currentUser?.permissions?.includes('USER_DELETE');
  ...
  const filteredUsers = useMemo(() => {
    ...
    /*
     * TASK 1:
     * Remove duplicate employee records before displaying them.
     *
     * Employee ID is treated as the primary unique identifier.
     * If employeeId is unavailable, fall back to id, _id, or email.
     */
    const seen = new Set();
    ...
    const uniqueUsers = users.filter((u) => {
      ...
      // If the record has no usable identifier,
      // keep it rather than accidentally removing valid data.
      if (!uniqueKey) {
        return true;
      }
                    ...
                    >
                      ...
                      >
                        ...
                        <button
                          type="button"
                          title="View Details"
                          aria-label={`View details for employee ${
                            user.fullName ||
                            user.email
                          }`}
                          onClick={() =>
